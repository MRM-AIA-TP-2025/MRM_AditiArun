...
def quaternion_to_euler_client(x, y, z, w):
    rospy.wait_for_service('toggle_imu_representation')
    try:
        toggle_imu_representation = rospy.ServiceProxy('toggle_imu_representation', ToggleIMURepresentation)
        response = toggle_imu_representation(x,y,z,w)
...
class RoverDisplay(QMainWindow):
    ...
    def imu_callback(self, msg):
        ...
        if self.orientation_representation == "quaternion":
            ...
            self.roll,self.pitch,self.yaw=quaternion_to_euler_client(msg.orientation.x, msg.orientation.y, msg.orientation.z,msg.orientation.w)
            orientation = f"roll: {self.roll:.2f}, pitch: {self.pitch:.2f}, yaw: {self.yaw:.2f}"
